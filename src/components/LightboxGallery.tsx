...
export default function LightboxGallery({ images, thumbnailWidth = '800px', thumbnailHeight }: LightboxGalleryProps) {
  ...
  // Scroll carousel by button click
  ...
  const endDrag = useCallback(() => {
    if (carouselRef.current) {
      ...
      // Reset drag flag after a short delay so click can check it
  ...
  // Drag to scroll - add mouse event listeners
  useEffect(() => {
  ...
  // Sync lightbox state with URL hash
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash;
      if (hash.startsWith('#lightbox-')) {
        const id = hash.substring(9); // Remove '#lightbox-'
        ...
        >
          {images.map((img, index) => (
            <a
              ...
              onAuxClick={(e) => {
                // Prevent middle-click (wheel) from opening in new tab
                e.preventDefault();
              }}
              style={{ flexShrink: 0 }}
            >
